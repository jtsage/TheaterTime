...
const outputConfig = () => {
	safeSend( 'config', dataStack.config )
}
const outputStatus = () => {
	safeSend( 'status', dataStack.status )
}
const outputUpdate = () => {
	safeSend( 'update', dataStack.update )
}
const outputLogger = () => {
	safeSend( 'log',    dataStack.logStack )
}
const configChange = () => {
	outputConfig(); outputStatus()
}
...
app.whenReady().then( () => {
	...
	ipcMain.on( 'logAudio', ( _, text : string, level : number ) => {
		dataStack.log( 'audioSystem', text, level )
	} )

	ipcMain.on( 'voiceTest', () => {
		dataStack.speakStack.push( 'Testing.  Testing, 1. 2. 3.' )
	} )

	ipcMain.on( 'switch:save', ( _, data : SwitchDefConfig[] ) => {
		dataStack.toggle.clear()
		dataStack.toggle.add_stack( data )
		configChange()
	} )
	ipcMain.on( 'switch:remove', ( _, index : number ) => {
		dataStack.toggle.remove( index )
		configChange()
	} )
	ipcMain.on( 'switch:toggle', ( _, index : number ) => {
		dataStack.toggleSwitch( index )
		outputStatus()
	} )

	ipcMain.on( 'timer:save', ( _, data : TimerDef[] ) => {
		dataStack.timers.clear()
		dataStack.timers.add_stack( data )
		configChange()
	} )
	ipcMain.on( 'timer:remove', ( _, index : number ) => {
	...
	ipcMain.on( 'settings', ( _, settings : TTSettings ) => {
	...
	ipcMain.on( 'config:load_drop', ( _, file : string ) => {
		try {
		...
		submenu : [
			{
				...
				submenu : [
					{ label : 'New Blank Config', click : () => {
						dataStack.defaultEmpty(); configChange()
					} },
					{ label : 'New from Rehearsal Template', click : () => {
						dataStack.defaultRehearsal(); configChange()
					} },
					{ label : 'New from Show Template', click : () => {
						dataStack.defaultShow(); configChange()
					} },
				],
			},
			{ type : 'separator' },
...
function doOSC( packet : Buffer ) {
	try {
		...
		if ( !oscPacket.address.startsWith( '/theaterTime' ) ) {
			return
		}
...
function oscToggle() {
	if ( dataStack.toggle.config.length === 0 ) {
		return
	}
