...
export const SwitchStatus = Object.freeze( {
	INACTIVE : 0,
	ACTIVE   : 1,
} )
...
export type SwitchDefConfig = SwitchDef & {
	id : string;
}

export type SwitchDefSerial = SwitchDef & {
	id : string;
	status : number;
}
...
class Switch implements SwitchInterface {
	...
	on()  {
		this.status = SwitchStatus.ACTIVE
	}
	
	off() {
		this.status = SwitchStatus.INACTIVE
	}

	get isOn() {
		return this.status === SwitchStatus.ACTIVE
	}
...
export class SwitchStack implements SwitchStackInterface {
...
export const DefaultShow : Array<SwitchDef> = [
	{
		reset_switches : [],
		reverseColor   : false,
...
export const DefaultRehearsal : Array<SwitchDef> = [
	{
		reset_switches : [],
		reverseColor   : false,
		speak          : 'Microphones are now ready.',
