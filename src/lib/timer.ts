...
export const TimerType = Object.freeze( {
	DOWN      : 2,
	MINUTES   : 3,
	UNDEFINED : 0,
	UP        : 1,
...
export type TimerDefUpdate = {
	uuid    : string | null;
	status  : number;
	speak ? : string | null;
}

export type TimerDefOSC = TimerDefUpdate & {
	formatTime   ? : string;
	title          : string | null;
	type           : number;
	wholeSeconds ? : number;
...
export type TimerUpdateToRender = {
	uuid           : string;
	status         : number;
	speak ?        : string | null;
	formatTime   ? : string;
...
export class TimerStack {
	...
	add_stack( stack : Array<Partial<TimerSave>> ) {
	...
	next() {
		if ( !this.#init || this.current_timer >= this.#stack.length ) {
			return
		}
...
class TimerDown extends TimerSTD implements TimerInterface {
	constructor( title : string | null, target : Date | string | null, sound_countdowns : boolean = false, reset_switches : string[] | null = null, sound_extra : string = '' ) {
		super( title, reset_switches )
	...
	get time() {
		switch ( this.status ) {
			case TimerStatus.PENDING : return 0
			case TimerStatus.FINISHED : {
				if ( this.targetDateTime === null || this.dateStopped === null ) {
					return 0
				}
