...
import type { SwitchStack, SwitchDefConfig } from "./switch";
import type { TimerStack, TimerDef } from "./timer";
import Timers   from './timer'
import Switches from './switch'

type TTSettings = {
	audio   : Record<string, any>;
	receive : Record<string, any>;
	send    : Record<string, any>;
}

type TTSaveFile = {
	timers : TimerDef[];
	toggle : SwitchDefConfig[];
	settings : TTSettings;
}
...
class DataStack {
	speakStack : string[] = []
	logStack : [string, string, number, string][]   = []
	timers : TimerStack
	toggle : SwitchStack
	...
	log(process : string, text : string, level : number) {
		const now = new Date()
	...
	get update() {
		...
		for ( const timer of timers ) {
			if ( typeof timer.speak === 'string' ) { this.speakStack.push(timer.speak) }
		}
		return {
			timers : timers,
